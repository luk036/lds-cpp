...
- [lds](https://github.com/luk036/lds)
...
To collect code coverage information, run CMake with the `-DLDS_ENABLE_COVERAGE=1` option.
...
```bash
cmake -B build -DLDS_BUILD_DOCS=ON
...
# run standalone
./build/Lds --help
# build docs (requires -DLDS_BUILD_DOCS=ON at configure time)
cmake --build build --target GenerateDocs
```

### Static analysis

clang-tidy can be enabled by configuring CMake with `-DLDS_ENABLE_CLANG_TIDY=ON` and building the `clang-tidy` target.
It analyzes the public headers using the checks configured in `.clang-tidy`.

```bash
cmake -B build -DLDS_ENABLE_CLANG_TIDY=ON
...
[![Star History Chart](https://api.star-history.com/svg?repos=luk036/lds-cpp,cpp-best-practices/gui_starter_template,filipdutescu/modern-cpp-template&type=Date)](https://star-history.com/#luk036/lds-cpp&cpp-best-practices/gui_starter_template&filipdutescu/modern-cpp-template&Date)
